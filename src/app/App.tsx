...
import { getCurrentSupabaseUser, signInWithEmail, signUpWithEmail } from "../services/authService";
...
function pageFromPath(pathname: string): Page {
  ...
  const match = Object.entries(pageRoutes).find(([, path]) => path === normalizedPath);
...
function LoginScreen({ onLogin }: { onLogin: () => void }) {
  ...
  const handleLogin = async () => {
    ...
    if (!isAdmin && !isRegistered && isSupabaseConfigured) {
      const result = await signInWithEmail(email, password);

      if (result.user) {
        localStorage.setItem("remedy_logged_in", "true");
        localStorage.setItem("remedy_is_demo", "false");
        localStorage.setItem("remedy_user_name", result.user.email?.split("@")[0] || "Kullanıcı");
        localStorage.setItem("remedy_user_email", result.user.email || email);
        localStorage.setItem("remedy_user_role", "Analist");
        restoreDataForUser(result.user.email || email);
...
export default function App() {
  ...
  console.log("pathname:", window.location.pathname, "activePage:", activePage);
  ...
  useEffect(() => {
    const restoreSession = async () => {
      ...
      if (user) {
        localStorage.setItem("remedy_logged_in", "true");
        localStorage.setItem("remedy_user_email", user.email || "kullanici@remedy.com");
        localStorage.setItem("remedy_user_name", user.email?.split("@")[0] || "Kullanıcı");
      }

      const logged = localStorage.getItem("remedy_logged_in");
      const hasSession = logged === "true" || Boolean(user);
