...
function getCategory(text: string): string {
  ...
  if (t.includes("kaza") || t.includes("fren") || t.includes("hava yastığı") || t.includes("hava yastigi") || t.includes("emniyet") || t.includes("ani durma")) return "Güvenlik";
  if (t.includes("akü") || t.includes("aku") || t.includes("batarya") || t.includes("pil") || t.includes("menzil")) return "Batarya";
  if (t.includes("şarj") || t.includes("sarj") || t.includes("trugo") || t.includes("dc") || t.includes("istasyon")) return "Şarj";
  if (t.includes("yazılım") || t.includes("yazilim") || t.includes("güncelleme") || t.includes("guncelleme") || t.includes("adas") || t.includes("truemore") || t.includes("dijital anahtar") || t.includes("uygulama")) return "Yazılım / Uygulama";
  if (t.includes("servis") || t.includes("randevu") || t.includes("müşteri hizmetleri") || t.includes("musteri hizmetleri") || t.includes("ulaşılamıyor") || t.includes("ulasilamiyor") || t.includes("çağrı") || t.includes("cagri")) return "Servis Süreci";
  if (t.includes("teslim") || t.includes("tescil") || t.includes("sipariş") || t.includes("siparis") || t.includes("gecikme") || t.includes("bayi")) return "Teslimat";
  if (t.includes("ekran") || t.includes("donanım") || t.includes("donanim") || t.includes("ayna") || t.includes("sensör") || t.includes("sensor") || t.includes("kamera") || t.includes("kapı") || t.includes("kapi")) return "Ekran / Donanım";
...
export function OverviewPage() {
  ...
  const actions = rows.filter((r) => r.riskScore >= 6.5).length;
      ...
      <div className="grid grid-cols-5 gap-4">
        <div className="col-span-3 rounded-xl" style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
          <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "1px solid #F1F5F9" }}>
            ...
            <button className="flex items-center gap-1 text-xs" style={{ color: "#1E5AA8" }}>
                ...
                recentActivity.map((row) => {
                  ...
                  return (
                    <tr key={row.id} style={{ borderTop: "1px solid #F1F5F9" }}>
                      ...
                      <td className="px-5 py-2.5" style={{ fontSize: "12px", fontWeight: 700, color: row.risk >= 7 ? "#DC2626" : row.risk >= 5 ? "#D97706" : "#16A34A" }}>
                        {row.risk}
                      </td>
