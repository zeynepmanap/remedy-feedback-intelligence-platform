...
import { classifyCategory as classifyRiskCategory, classifyComplaintRisk, compareFirstFiveRiskScores, createRiskContext } from "../../utils/riskScoring";
...
function getResponsibleUnit(category: string) {
  switch (category) {
    ...
    case "Teslimat":
      return "Operasyon";
    case "Fiyat":
      return "Finans & Müşteri Deneyimi";
    case "Ekran / Donanım":
...
function getRootCause(category: string) {
  switch (category) {
    ...
    case "Fiyat":
      return "Fiyatlandırma, kampanya, ödeme koşulları veya fatura iletişiminde beklenti uyumsuzluğu.";
    case "Ekran / Donanım":
...
function getRecommendedAction(category: string) {
  switch (category) {
    ...
    case "Fiyat":
      return "Fiyat, ödeme, fatura ve kampanya açıklamaları gözden geçirilmeli; müşteriye şeffaf bilgilendirme yapılmalıdır.";
    case "Ekran / Donanım":
...
function getDescription(category: string, count: number): string {
  switch (category) {
    ...
    case "Fiyat":
      return `${count} kayıt fiyat, ödeme, fatura veya kampanya beklentilerinde açıklık ihtiyacını gösteriyor.`;
    case "Ekran / Donanım":
...
function getSubIssues(category: string): string[] {
  switch (category) {
    ...
    case "Fiyat":
      return ["Fiyat algısı", "Ödeme", "Fatura", "Kampanya"];
    case "Ekran / Donanım":
...
function convertCSVToRiskCards(csv: string): RiskCard[] {
  ...
  const fullTexts = dataRows
    .map((cols) => (hasHeader ? pickBestText(cols, header) : cols.filter(Boolean).join(" ")))
    .filter((text) => text && text.trim().length >= 4);
  const riskContext = createRiskContext(fullTexts);

  compareFirstFiveRiskScores(
    "RiskPage",
    fullTexts.slice(0, 5).map((fullText) => ({
      text: fullText,
      score: classifyComplaintRisk(fullText, riskContext).riskScore,
    }))
  );
  ...
  return Object.entries(grouped)
    .map(([category, info], index) => {
      ...
      const score = Number(Math.min(avgRisk + volumeEffect, 10).toFixed(1));
      ...
      return {
        ...
        riskScore: score,
        totalRisk: info.totalRisk,
        complaints: info.count,
       trend: score >= 8.5 ? "up" : score >= 7 ? "stable" : "down",
trendValue:
  score >= 8.5
    ? `+${Math.round(score)}%`
    : score >= 7
    ? `+${Math.round(score / 2)}%`
    : `-${Math.max(1, Math.round(7 - score))}%`,
        impact,
        priority,
...
function RiskGauge({ score }: { score: number }) {
  const color =
    score >= 8.5 ? "#DC2626" : score >= 7 ? "#D97706" : score >= 4 ? "#F59E0B" : "#16A34A";
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <div style={{ fontSize: "11px", fontWeight: 600, color }}>
          {score >= 8.5 ? "Kritik" : score >= 7 ? "Yüksek" : score >= 4 ? "Orta" : "Düşük"}
        </div>
      </div>
    </div>
  );
...
export function RiskPage() {
  ...
  const avgScore =
    totalComplaints > 0
      ? riskCards.reduce((sum, c) => sum + c.totalRisk, 0) / totalComplaints
      : 0;
