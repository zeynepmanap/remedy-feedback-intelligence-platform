...
const fallbackTitle = "Örnek şikayet kaydı";
const fallbackText = "CSV yüklenmediği için örnek veri gösteriliyor.";
const fallbackRisk = classifyComplaintRisk(`${fallbackTitle} ${fallbackText}`);

const fallbackData: FeedbackRow[] = [
  {
    id: "FB-0001",
    brand: "DEMO",
    category: fallbackRisk.category,
    riskScore: fallbackRisk.riskScore,
    priority: fallbackRisk.priority,
    status: fallbackRisk.riskScore >= 8.5 ? "İşlemde" : fallbackRisk.riskScore >= 7 ? "Planlandı" : "Tamamlandı",
...
function getAction(category: string): string {
  switch (category) {
    ...
    case "Fiyat":
      return "Fiyat, ödeme ve kampanya iletişimi netleştirilerek müşteri bilgilendirme süreci güçlendirilsin.";
    case "Ekran / Donanım":
...
function convertCSVToRows(csv: string, brand: string): FeedbackRow[] {
  ...
  const rows = dataRows
    .map((cols, index) => {
      ...
      return {
        ...
        riskScore,
        priority,
        status: riskScore >= 8.5 ? "İşlemde" : riskScore >= 7 ? "Planlandı" : "Tamamlandı",
  ...
  compareFirstFiveRiskScores(
    "AnalysisPage",
    rows.slice(0, 5).map((row) => ({ text: `${row.title} ${row.text}`, score: row.riskScore }))
  );
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        style={{
                          fontSize: "13px",
                          fontWeight: 800,
                          color: row.riskScore >= 8.5 ? "#DC2626" : row.riskScore >= 7 ? "#D97706" : row.riskScore >= 4 ? "#1E5AA8" : "#16A34A",
                        }}
                      >
                      ...
                      <div className="w-16 h-1.5 rounded-full" style={{ background: "#F1F5F9" }}>
                        <div
                          ...
                          style={{
                            ...
                            background: row.riskScore >= 8.5 ? "#DC2626" : row.riskScore >= 7 ? "#F59E0B" : row.riskScore >= 4 ? "#1E5AA8" : "#16A34A",
                          }}
                        />
                      </div>
                    </div>
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      color:
                        label === "Risk Skoru"
                          ? selectedRow.riskScore >= 8.5
                            ? "#DC2626"
                            : selectedRow.riskScore >= 7
                            ? "#D97706"
                            : selectedRow.riskScore >= 4
                            ? "#1E5AA8"
                            : "#16A34A"
                          : "#0F172A",
                    }}
