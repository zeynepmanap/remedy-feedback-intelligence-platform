...
function getPriority(avgRisk: number, count: number) {
  if (avgRisk >= 8 || count >= 20) return "Çok Yüksek";
  if (avgRisk >= 7 || count >= 10) return "Yüksek";
  if (avgRisk >= 5.5 || count >= 5) return "Orta";
  return "Düşük";
}
...
export function ReportingPage() {
  ...
  const stats = useMemo(() => {
    ...
    const criticalRisk = rows.filter((r) => r.risk >= 8).length;
