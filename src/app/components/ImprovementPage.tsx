...
import { classifyComplaintRisk, getRiskPriority } from "../../utils/riskScoring";
...
const baseActions: Omit<ActionCard, "riskScore">[] = [
  {
    id: 1,
    category: "Batarya",
...
const actions: ActionCard[] = baseActions.map((action) => ({
  ...action,
  riskScore: classifyComplaintRisk(`${action.category} ${action.title} ${action.description}`).riskScore,
}));
...
const riskColor = (score: number) => {
  ...
  if (priority === "Kritik") return "#dc2626";
  if (priority === "Yüksek") return "#d97706";
  if (priority === "Orta") return "#2563eb";
  return "#16a34a";
};
